...
void NewOrder::generatePicture()
{
    ...
    QRegularExpression steelRegex("(\\d+): MOVE-STEEL-CONTAINER (\\w+) (\\w+) (\\w+) (\\w+)");
    steelRegex.setPatternOptions(QRegularExpression::MultilineOption);
    ...
    while (!in.atEnd())
    {
        ...
        QRegularExpressionMatch steelMatch = steelRegex.match(line);
        ...
        if (steelMatch.hasMatch())
        {
            //d动作序号 w 钢材名称  w 货箱  w 车辆  w 地点

            // 获取匹配的操作行号和司机名字
            QString lineNumber = steelMatch.captured(1);
            QString steelType = steelMatch.captured(2);
            QString truckType = steelMatch.captured(4);
            QString yardName = steelMatch.captured(5);

            // 检查车辆是否已经在Map中
            if (!loadActions.contains(truckType))
            {
                loadActions.insert(truckType, QStringList());
...
//            QString nextActionLineNumber = "";
//            if( (i+4) > actions.size())
//            {
//                nextActionLineNumber = "";
//            }
//            else
//            {
//                nextActionLineNumber = actions[i + 4];
//            }
            ...
            steelTypeList.clear();
            getSteelType(driveTruck,steelTypeList);
...
void NewOrder::getSteelType(QString driveTruck, QStringList & steelTypeList)
{
    int startLine = 0,endLine = 0,planLength = 0,nextLineNumber = 0;
    positioningPlan(startLine,endLine,planLength);
    //int lineNumber = actionLineNumber.toInt();
//    if(nextActionLineNumber == "")
//    {
//        nextLineNumber = planLength;
//    }
//    else
//    {
//        nextLineNumber = nextActionLineNumber.toInt();
//    }

    // 获取该driveTruck对应的actions
    QStringList actions = loadActions.value(driveTruck);
    // 遍历actions，查找包含特定truckType的项目
    for (int i = 0; i < actions.size(); i += 3)
    {
        if (actions[i] == driveTruck)
        {
            //int loadLineNumber = actions[i+1].toInt();

            QString steelType = actions.at(i+2);
